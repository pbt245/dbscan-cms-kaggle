...
def iterative_meanshift_dbscan(model, loader, args):
    """
    DBSCAN clustering with iterative mean-shift feature refinement
    """
    print(f'Using DBSCAN with eps={args.eps}, min_samples={args.min_samples}')
    ...
    with torch.no_grad():
        for epoch in range(args.epochs):
            ...
            # For normalized vectors, cosine distance = (1 - cosine_similarity) / 2
            # But we can also use euclidean on normalized vectors
            clustering = DBSCAN(
                eps=args.eps,
                min_samples=args.min_samples,
                metric='euclidean',  # On normalized vectors, this is related to cosine
                n_jobs=-1
            )
            ...
            if n_clusters == 0:
                print('No clusters found! Try increasing eps or decreasing min_samples')
                break
            ...
            if n_noise > 0:
                ...
                if valid_mask.sum() > 0:
                    # Compute distances from noise points to cluster centroids
            ...
            all_acc_test, old_acc_test, new_acc_test = log_accs_from_preds(
                ...
                save_name='DBSCAN unlabeled train ACC',
                print_output=True
            )
    ...
    print(f'Unlabeled test - All: {best_result[0]:.4f} | Old: {best_result[1]:.4f} | New: {best_result[2]:.4f}')
    
    return best_result


def iterative_meanshift_dbscan_inductive(model, loader, val_loader, args):
    """
    DBSCAN clustering for inductive setting
    """
    print(f'Using DBSCAN with eps={args.eps}, min_samples={args.min_samples}')
        ...
        for epoch in range(args.epochs):
            ...
            for batch_idx, batch in enumerate(tqdm(val_loader, desc='Val features')):
                ...
                if epoch == 0:
                    ...
                    classwise_sim = torch.einsum('b d, n d -> b n', 
                                                 torch.cat(all_feats_val)[start_idx:end_idx],
                                                 torch.cat(all_feats_val))
                    _, indices = classwise_sim.topk(k=args.k+1, dim=-1, largest=True, sorted=True)
                    indices = indices[:, 1:]
                    knn_emb_val = torch.mean(
                        torch.cat(all_feats_val)[indices].view(-1, args.k, args.feat_dim), dim=1
                    )
                    new_feats_val[start_idx:end_idx] = (
                        (1-args.alpha) * torch.cat(all_feats_val)[start_idx:end_idx] + 
                        args.alpha * knn_emb_val.detach().cpu()
                    )
            ...
            if epoch == 0:
                ...
                norm = torch.sqrt(torch.sum(torch.pow(new_feats_val, 2), dim=-1, keepdim=True))
                new_feats_val = new_feats_val / (norm + 1e-8)
                all_feats_val = new_feats_val

            # DBSCAN on test and validation
            ...
            # Cluster test data
            clustering_test = DBSCAN(eps=args.eps, min_samples=args.min_samples, 
                                    metric='euclidean', n_jobs=-1)
            preds_test = clustering_test.fit_predict(feats_test_np)
            
            # Cluster validation data  
            clustering_val = DBSCAN(eps=args.eps, min_samples=args.min_samples,
                                   metric='euclidean', n_jobs=-1)
            preds_val = clustering_val.fit_predict(feats_val_np)
            
            # Handle noise points for both
            for preds, feats_np, name in [(preds_test, feats_test_np, 'test'), 
                ...
                print(f'{name}: {len(unique_labels)} clusters, {n_noise} noise reassigned')
            
            # Evaluate
            from project_utils.cluster_utils import cluster_acc
            old_acc_val = cluster_acc(targets_val[mask_cls_val], preds_val[mask_cls_val])
    ...
    print(f'Test - All: {best_result[0]:.4f} | Old: {best_result[1]:.4f} | New: {best_result[2]:.4f}')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--pretrain_path', type=str, default='/kaggle/working/dbscan-for-cms/models')

    
    # DBSCAN specific parameters
    parser.add_argument('--eps', type=float, default=0.15, 
                       help='DBSCAN eps parameter (try 0.1-0.3 for normalized features)')
    ...
    # Load model
    model_path = f"./log/metric_learn_gcd/log/{args.model_name}/checkpoints/model_best.pt"
    ...
    # Load data
    from data.augmentations import get_transform
    train_transform, test_transform = get_transform('imagenet', image_size=224, args=args)
    ...
    if args.inductive:
        ...
        iterative_meanshift_dbscan_inductive(model, test_loader, val_loader, args)
    else:
        ...
        train_loader = DataLoader(train_dataset, batch_size=args.batch_size, 
                                  num_workers=args.num_workers)
        iterative_meanshift_dbscan(model, train_loader, args)
